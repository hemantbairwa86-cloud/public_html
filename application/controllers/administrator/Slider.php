...
class Slider extends MY_Controller  {
	...
	{	
		...
		{
			$this->form_validation->set_rules('name', 'Slider Title', 'required|trim');
	...
	public function file_check($str){
        ...
        if(isset($_FILES['image']['name']) && $_FILES['image']['name']!=""){
            if(in_array($mime, $allowed_mime_type_arr))
            {
            	 ...
            	 {
            	 	$this->form_validation->set_message('file_check', '<b>Error : </b>Invaliad Width / Height.');
                	return false;
            	 }else
            	 { 
                	return true;
                ...
                $this->form_validation->set_message('file_check', '<b>Error : </b>Please select only jpg/png file.');
                return false;
            }
        }else{
            $this->form_validation->set_message('file_check', '<b>Error : </b>Please choose a file to upload.');
            return false;
        }
    }
	...
	public function file_check_edit($str){
	        ...
	        if(isset($_FILES['image']['name']) && $_FILES['image']['name']!=""){
	            if(in_array($mime, $allowed_mime_type_arr))
	            {
	            	 ...
	            	 if ($width != 1920 || $height != 680 )
	            	 {
	            	 	$this->form_validation->set_message('file_check_edit', '<b>Error : </b>Invaliad Width / Height.');
	                	return false;
	            	 }else
	            	 { 
	                	return true;
	                ...
	                $this->form_validation->set_message('file_check_edit', '<b>Error : </b>Please select only jpg/png file.');
	                return false;
	            }
	        }else{
	            return true;
