...
class order extends MY_Controller {
	...
	public function view_order_ajax_data()
	{
		...
		$tot_rec=$this->db->select("id")->from('customer_bill')->get()->result_array();
        ...
        if(!empty($requestData['search']['value'])) {
        	$this->db->group_start();
            $this->db->or_like('customer_bill.series',$requestData['search']['value']);
            $this->db->or_like('customer_bill.billing_city',$requestData['search']['value']);
       ...
       // echo $this->db->last_query();			
        ...
        { 
            ...
            $nestedData = array();
            $nestedData[] = $k;
            $nestedData[] = $val['order_id']."<br>".$val['razorpay_payment_id'];
            $nestedData[] = date('d/m/Y',strtotime($val['oreder_date']));
			 $nestedData[] = $val['name']." ".$val['surname'];
            $nestedData[] = $val['billing_city'];
        //    $nestedData[] = $val['total_amount'];
            $nestedData[] = $val['final_total'];           
			...
			$nestedData[] = '<a  href="'.$mylink.'" target="_blank" class="btn btn-outline-primary mb-2" >Print</a>&nbsp;&nbsp;<a  href="'.$detail.'" class="btn btn-outline-success" >Detail</a>';            
            $data[] = $nestedData;
            $k++  ; 			
        }
        $json_data = array(
	...
	public function detail($id)
	{
		...
		if(!empty($OrderData))
		{
			...
			$this->load->view('administrator/order-detail',$data);
		}
		else
		{
			redirect('administrator/dashboard','refresh');
